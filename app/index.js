...
function startAssessmentService() {
  ...
  assessmentService.stdout.on('data', (data) => {
    console.log(`Assessment Service: ${data.toString().trim()}`);
  });

  assessmentService.stderr.on('data', (data) => {
    console.error(`Assessment Service Error: ${data.toString().trim()}`);
  });

  assessmentService.on('close', (code) => {
    console.log(`Assessment Service process exited with code ${code}`);
    assessmentService = null;
  });

  assessmentService.on('error', (error) => {
